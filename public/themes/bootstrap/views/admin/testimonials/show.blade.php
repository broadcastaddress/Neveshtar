			...
			<div class="portlet-body form">
				...
				<form action="/admin/{{$active}}/{{$item->id}}" method="POST" id="admin_form" class="form-horizontal">
					...
					<div class="form-body">
						...
						<div class="form-group">
							<label class="control-label col-md-3">{{Lang::get('admin.image')}} <span class="required">
							* </span>
							</label>
							<div class="col-md-4">
...
@if($item->image)
<script>
    $("#author_image").select2("data", { id: "{{$item->image->id}}", text: "{{$item->image->title}}", url: "{{$item->image->url}}" });
</script>
@endif
@endsection
